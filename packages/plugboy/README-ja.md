...
`splitting` は指定しないことを推奨します。`css: true` のエントリを持つワークスペースでは、plugboy がこれを有効にし、宣言したスタイルシートを依存順で自ら組み立てます（[CSS エントリのスタイルシート](#css-エントリのスタイルシート) を参照）。`fileName` はその出力名として使われます。`splitting` を指定すると、結合は記述どおり tsdown に委ねられます。プラグインがワークスペースのセットアップ時に他のデフォルト値を与える場合もありますが、設定値は常にそれより優先されます。
...
この処理は `writeBundle` でディスク上のスタイルシートに対して実行されるため、ビルドが書き出す**すべての**スタイルシートが対象になります。tsdown 自身の CSS パイプラインは全プラグインの処理後に CSS を出力しますが、それも含まれます。

#### スタイルシート先頭で保持されるもの

tsdown の CSS パイプラインが書き換えてしまう次の 2 つは、すべてのルールより上に、この順で復元されます。

1. **記述された `@layer` の順序** — lightningcss は、同じスタイルシート内に対象レイヤーのブロックがある場合、`@layer a, b, c;` 文からその名前を冗長として削除します。しかしその宣言文が**他パッケージ**のレイヤー順まで規定している場合、これは誤りになります（削除された名前のレイヤー位置が、自身のブロックが他パッケージのスタイルシートに対してどこに来るかで決まってしまう）。plugboy は変換前に宣言文を読み取り、記述されたままの順序で再出力します。
2. **外部 `@import`** — パッケージ指定の `@import`（例: `@import url('material-symbols/rounded.css') layer(...)`）はインライン化せず external のまま保持します。利用側のバンドラが解決し、対象パッケージ自身の相対アセット URL も壊れません。

いずれもモジュールグラフ上のすべてのスタイルシートから取得します。内容が記述されたものか、プラグインが生成したものかは問いません（vanilla-extract は `@layer` 文を仮想モジュールへ出力しますが、それも対象です）。

#### CSS エントリのスタイルシート

`css: true` の各エントリには `./<entry>.css` の export が宣言されます。それらのファイルは必ず実体として存在し、単体で使え、かつ**依存順**に並んでいることが保証されます。つまり、あるスタイルが合成（compose）または import しているスタイルは、必ずそれより前に来ます。これにより、共有リセットや vanilla-extract の `style([base, { … }])` の `base` の上に書いたルールが、意図どおり `base` に勝ちます。

これらのファイルは plugboy 自身が組み立てます。ビルドは出力**チャンク**ごとにスタイルシートを出力し（`css.splitting`）、plugboy はそれをチャンクが読み込まれる順に連結します。この順序は Vite の `build.cssCodeSplit: false` と同じで、各チャンクは静的に import しているチャンクの後に、`import()` でしか到達しないチャンクはそれらすべての後に並びます。tsdown 自身の単一ファイルへの結合は使いません。バンドル順に連結するため、共有チャンクがそれに依存するエントリの**後**に来てしまうからです。

- **CSS エントリが 1 つ**: パッケージ唯一のスタイルシートに、`import()` で読み込まれるものも含めた全チャンクの CSS が入ります（`css.inject` が無効なら、チャンク単位のスタイルシートを実行時に読み込むものは何もないため）。`css.fileName` が指定されていればその名前で出力します。
- **複数**: 各エントリのスタイルシートには、自身の CSS と、そこから到達する全チャンクの CSS が入ります。エントリ間で共有される CSS はそれぞれに複製されるので、`./<entry>.css` 単体の import で完結します。

組み立てに使ったチャンク単位のファイルは、どの export も指していないため削除されます。

保証するのはチャンク単位の依存順です。Vite の開発サーバー（したがって Storybook）はモジュール単位でスタイルを差し込むので、その順序とは一致しません。互いに依存しない 2 つのルールは、相対的な順序が入れ替わることがあります。そうした 2 つが同じ要素に同じ詳細度で当たる場合は、`@layer` を分けるか一方を他方から合成するかして、どちらが勝つべきかを明示してください。

`css.inject` が有効な場合（JavaScript がチャンク単位のスタイルシートを名前で import するため、出力のまま残します）と、`splitting` を指定した場合（結合は tsdown に委ねます）は、組み立てを行いません。
