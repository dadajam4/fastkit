...
`css: true` の各エントリには `./<entry>.css` の export が宣言されます。それらのファイルは必ず実体として存在し、かつ単体で使える状態になることが保証されます。

該当エントリが複数ある場合、ビルドは出力**チャンク**ごとにスタイルシートを出力します（`css.splitting`）。これはエントリ単位とは一致しません。複数のエントリから参照される CSS は共有チャンクへ移され、どの export も指していないハッシュ名で出力されます。さらに、CSS がその共有チャンクにしか無いエントリはスタイルシートを一切持ちません。plugboy は各エントリのスタイルシートを「自身の CSS ＋ import しているチャンクの CSS（依存側が先）」から再構成し、残ったチャンク単位のファイルを削除します。共有 CSS は必要な各エントリへ複製されます。これにより `./<entry>.css` 単体の import で完結します。

`css.inject` が有効な場合はこの処理を行いません（JavaScript がチャンク単位のスタイルシートを名前で import するため）。
