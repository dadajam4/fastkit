...
describe('スキーマを定義する時', () => {
  ...
  describe('シンプルなスキーマを定義する時', () => {
    ...
    it('スキーマからスペースのコンストラクタを取得できること', () => {
      expect(schema.Space).toBe(Space);
    });

    it('依存コンポーネントを指定しなかった時、依存コンポーネントマップが空のマップであること', () => {
      expect(schema.dependencies).toStrictEqual({});
    });

    it('指定した日時フォーマットオプションが取得できること', () => {
      expect(schema.dateTimeFormats.d).toStrictEqual(d);
    });

    it('指定した数値フォーマットオプションが取得できること', () => {
      expect(schema.numberFormats.n).toStrictEqual(n);
    });

    it('指定したリストフォーマットオプションが取得できること', () => {
      expect(schema.listFormats.l).toStrictEqual(l);
    });
  });
});
