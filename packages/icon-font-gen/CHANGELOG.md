...
- [#202](https://github.com/dadajam4/fastkit/pull/202) [`71bba8f`](https://github.com/dadajam4/fastkit/commit/71bba8fdfb1dbaeb0f337814c0f067402f294cb5) Thanks [@dadajam4](https://github.com/dadajam4)! - Remove the `src: '@mdi'` sentinel, and with it the build-time `pnpm add`.
  ...
  Passing `'@mdi'` is now an error naming the replacement, rather than being left to fail on its own: an entry whose source directory does not exist is _skipped_, so the old behaviour of this input would have been a silently empty font.
